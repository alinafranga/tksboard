...
@Component({
  selector: 'app-producer-gr-product-lists',
  templateUrl: './producer-gr-product-lists.component.html',
  styleUrls: ['./producer-gr-product-lists.component.css']
})
export class ProducerGrProductListsComponent implements OnInit, OnChanges {
  ...
  private _getOverviewAnalyticsInfo() {
    if(this.date != ''){
      this._producerGraphicService.getOverviewByProductAnalytics(this.weedId, this.provinceId, this.date).subscribe((data: OverviewMetricsInfo) => {
        this.listsDifference = data.Lists.TotalPreviousPeriodPercentage;
      });
    }
  }
  ...
  private _loadData() {
    this.loading = true;
    if(this.date != ''){
      this._producerGraphicService.getProductLists(this.weedId, this.provinceId, this.date)
